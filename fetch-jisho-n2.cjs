...
// We will fetch "#jlpt-n2"
const searchQuery = encodeURIComponent('#jlpt-n2');
let page = 1;
const maxPages = 60; // 60 pages * 20 words = 1200 words approx
const results = [];
let idCounter = 1;
...
async function run() {
  console.log('Fetching N2 vocabulary from Jisho...');
  ...
  const outPath = path.join(__dirname, 'src', 'data', 'kotobaN2.json');
